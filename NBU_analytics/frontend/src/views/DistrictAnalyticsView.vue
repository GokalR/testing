          ...
          <div class="col-span-12 lg:col-span-7 da-card">
            <div class="flex items-start justify-between">
              ...
              <div class="flex gap-3 text-[10px] font-bold uppercase tracking-wider">
                <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-sm" style="background:linear-gradient(90deg,#003D7C,#2563EB)"></span>{{ t('district.cards.district') }}</span>
                <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-sm" style="background:linear-gradient(90deg,#0891B2,#22D3EE)"></span>{{ t('district.cards.benchmark') }}</span>
              </div>
            </div>
            <div class="mt-4">
              <div v-for="row in analytics.economic.benchmark.rows" :key="row.label" class="da-bench-row">
                ...
                <div class="da-bench-bar">
                  <span class="da-bench-bar-label text-primary">{{ t('district.cards.district') }}</span>
                ...
                <div class="da-bench-bar">
                  <span class="da-bench-bar-label" style="color:#0891B2">{{ t('district.cards.benchmark') }}</span>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-5">
            <div v-for="p in analytics.infra.problems" :key="p.code" class="border border-outline-variant/30 rounded-xl p-4 hover:shadow-md hover:border-primary/30 transition-all">
              <div class="flex items-center justify-between">
                ...
                <span class="da-chip" :class="p.priority === 'высокий' ? 'tone-red' : p.priority === 'средний' ? 'tone-amber' : 'tone-blue'">{{ t(`district.cards.priority${p.priority === 'высокий' ? 'High' : p.priority === 'средний' ? 'Medium' : 'Low'}`) }}</span>
              </div>
