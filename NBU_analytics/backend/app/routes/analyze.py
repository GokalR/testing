...
from ..models import AnalysisResult, Submission
...
def _build_context(sub: Submission) -> dict:
    """Assemble the payload sent to Claude.

    Includes: user profile + finance, resolved city context (or unsupported marker),
    user-extracted ratios from the most recent upload (if any), and peer-median
    comparison from peer_benchmarks.json.
    """
    city = cities.resolve(sub.city_id, sub.profile)

    # Pull the most recent ratio computation from the latest upload, if any.
    user_ratios = None
    user_absolutes = None
