"""
Loads peer_benchmarks.json once at startup and exposes a comparison helper.
Data file is copied from frontend/src/data/regionalStrategist/peer_benchmarks.json
into app/data/ so the backend is self-contained.
"""
...
_DATA = Path(__file__).resolve().parent.parent / "data" / "peer_benchmarks.json"
...
def compare(user_ratios: dict[str, float | None]) -> list[dict]:
    """Returns [{key, user, q1, median, q3, percentileHint}]."""
