...
Builds a compact, structured prompt from:
  - user profile + finance (Vue store blocks)
  - resolved city context (services/cities.py)
  - user's computed ratios + peer benchmarks comparison

Asks Claude to return strict JSON matching the shape declared in SYSTEM_PROMPT.
"""
...
def build_user_message(context: dict[str, Any]) -> str:
    """Compact, structured user message. Keep tokens low."""
    return "ВХОД:\n" + json.dumps(context, ensure_ascii=False, indent=2)


def analyze(context: dict[str, Any], lang: str = "ru", model: str | None = None) -> dict[str, Any]:
    """
    Calls Anthropic. Returns:
      {"output": {...parsed JSON...}, "input_tokens": N, "output_tokens": N, "model": "..."}
    Raises RuntimeError on client/parse failures (let the endpoint log + persist).
    """
