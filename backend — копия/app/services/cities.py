...
Mirrors frontend/src/data/regionalStrategist/cities.js at the data level.

Only Fergana + Margilan are pilot cities with real data. For any other
region we return an explicit "unsupported" marker so the frontend and
the prompt can both say "data not available yet" instead of silently
falling back to Fergana.
"""
...
def resolve(city_id: str | None, profile: dict | None = None) -> dict:
    """Return city context. For non-pilot regions, returns {supported: False, note: ...}."""
    ...
    # Non-pilot: surface the user's own label if any.
