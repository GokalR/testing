...
from ..models import ExcelUpload, Submission
...
async def upload_excel(
    ...
    # Compute ratios now if we have both forms (so GET is cheap later).
    combined_form1 = parsed_codes if kind == "balance" else None
    combined_form2 = parsed_codes if kind == "pnl" else None

    # Merge with the other side if already uploaded.
    ...
    rec = ExcelUpload(
        ...
        raw_blob=None,  # privacy: don't persist raw by default
    )
    db.add(rec)
    db.commit()
    db.refresh(rec)
