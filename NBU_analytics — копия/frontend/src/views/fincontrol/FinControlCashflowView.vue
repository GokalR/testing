...
import { recurring, monthly } from '@/data/fincontrol'
...
const cfData = {
  labels: days,
  datasets: [
    { type: 'bar', label: 'Приход', data: inflowDay, backgroundColor: '#003D7C', borderRadius: 4, barThickness: 8 },
    { type: 'bar', label: 'Расход', data: outflowDay.map((v) => -v), backgroundColor: '#CBD5E8', borderRadius: 4, barThickness: 8 },
    {
...
<template>
  ...
  <div class="fc-content">
    <div class="fc-page-title">
      <div class="eyebrow">FinControl / Cash Flow</div>
      <h1>Движение денежных средств</h1>
      <p class="subtitle"><span class="fc-green-line"></span>Апрель 2026 · ООО «Глобал Трейд Групп»</p>
    </div>

    <!-- KPI strip -->
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:18px">
      <div class="fc-kpi" style="border-top:3px solid #00A651">
        <div class="fc-kpi-label">Входящий поток</div>
        <div class="fc-kpi-value green">320 000 000 <span style="font-size:12px;opacity:.55">UZS</span></div>
        <div class="flex items-center gap-2" style="margin-top:6px"><span class="fc-badge green">▲ 8.2%</span><span class="fc-kpi-sub">vs прошлый месяц: 295 700 000</span></div>
      ...
      <div class="fc-kpi" style="border-top:3px solid #E0384B">
        <div class="fc-kpi-label">Исходящий поток</div>
        <div class="fc-kpi-value red">198 000 000 <span style="font-size:12px;opacity:.55">UZS</span></div>
        <div class="flex items-center gap-2" style="margin-top:6px"><span class="fc-badge red">▼ 3.1%</span><span class="fc-kpi-sub">vs прошлый месяц: 204 400 000</span></div>
      ...
      <div class="fc-kpi" style="border-top:3px solid #003D7C">
        <div class="fc-kpi-label">Чистый CF</div>
        <div class="fc-kpi-value navy">122 000 000 <span style="font-size:12px;opacity:.55">UZS</span></div>
        <div class="flex items-center gap-2" style="margin-top:6px"><span class="fc-badge blue">▲ 15.0%</span><span class="fc-kpi-sub">vs прошлый месяц: 106 100 000</span></div>
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:16px;margin-bottom:18px">
      <div class="fc-card">
        <div class="fc-card-header">
          <div>
            <div class="fc-card-title">График движения денег</div>
            <div class="fc-card-sub">Апрель 2026 · Нажмите на столбец для деталей</div>
          </div>
          <div class="flex items-center gap-3">
            <div style="display:flex;background:#F0F4FA;border-radius:8px;padding:2px">
              <button class="fc-period-pill" :class="{active:granularity==='day'}" @click="granularity='day'" style="font-size:11px;padding:4px 10px;border-radius:6px">По дням</button>
              <button class="fc-period-pill" :class="{active:granularity==='week'}" @click="granularity='week'" style="font-size:11px;padding:4px 10px;border-radius:6px">По неделям</button>
              <button class="fc-period-pill" :class="{active:granularity==='month'}" @click="granularity='month'" style="font-size:11px;padding:4px 10px;border-radius:6px">По месяцам</button>
            </div>
          </div>
        </div>
        <div style="display:flex;gap:14px;margin-bottom:8px;font-size:12px;color:#6B7A99">
          <span><span style="display:inline-block;width:10px;height:10px;background:#003D7C;border-radius:2px;margin-right:6px"></span>Приход</span>
          <span><span style="display:inline-block;width:10px;height:10px;background:#CBD5E8;border-radius:2px;margin-right:6px"></span>Расход</span>
        ...
        <div class="fc-card-sub" style="margin-top:8px;text-align:center"><AppIcon name="info" style="font-size:14px;vertical-align:middle" /> Нажмите на день чтобы увидеть транзакции</div>
      </div>

      <div class="fc-card">
        <div class="fc-card-header">
          <div>
            <div class="fc-card-title">CF по видам деятельности</div>
        ...
        <div style="display:flex;flex-direction:column;gap:14px">
          <div>
            <div class="flex items-center gap-2" style="margin-bottom:6px">
              ...
              <div style="flex:1"><div style="font-weight:700;font-size:13px;color:#1A2B4A">Операционный CF</div><div style="font-size:11px;color:#6B7A99">Основная деятельность</div></div>
          ...
          <div>
            <div class="flex items-center gap-2" style="margin-bottom:6px">
              ...
              <div style="flex:1"><div style="font-weight:700;font-size:13px;color:#1A2B4A">Инвестиционный CF</div><div style="font-size:11px;color:#6B7A99">Покупка оборудования</div></div>
          ...
          <div>
            <div class="flex items-center gap-2" style="margin-bottom:6px">
              ...
              <div style="flex:1"><div style="font-weight:700;font-size:13px;color:#1A2B4A">Финансовый CF</div><div style="font-size:11px;color:#6B7A99">Кредиты и займы</div></div>
          ...
          <div style="background:#EEF4FF;padding:12px;border-radius:9px;display:flex;align-items:center;justify-content:space-between">
            <div><div style="font-weight:800;font-size:13px;color:#003D7C">Чистый CF</div><div style="font-size:11px;color:#6B7A99">Сумма всех потоков</div></div>
    ...
    <div class="fc-card" style="padding:0;overflow:hidden">
      <div class="fc-card-header" style="padding:18px 18px 14px">
        <div>
          <div class="fc-card-title">Регулярные платежи</div>
          <div class="fc-card-sub">Запланированные и повторяющиеся операции</div>
        </div>
        <div style="display:flex;background:#F0F4FA;border-radius:8px;padding:2px">
          <button class="fc-period-pill" :class="{active:tab==='outgoing'}" @click="tab='outgoing'" style="font-size:11px;padding:4px 10px;border-radius:6px">Исходящие</button>
          <button class="fc-period-pill" :class="{active:tab==='incoming'}" @click="tab='incoming'" style="font-size:11px;padding:4px 10px;border-radius:6px">Входящие</button>
        </div>
      </div>
      <table class="fc-table">
        <thead>
          <tr><th>Название</th><th>Категория</th><th>Периодичность</th><th>Следующая дата</th><th style="text-align:right">Сумма</th><th>Статус</th></tr>
        </thead>
        <tbody>
