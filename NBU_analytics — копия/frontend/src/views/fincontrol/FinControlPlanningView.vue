...
const categories = ref([
  { key: 'purchases', name: 'Закупки', color: '#0054A6', plan: 55000000, fact: 61600000, tone: 'red', icon: 'inventory_2' },
  { key: 'salary', name: 'Зарплата', color: '#00A651', plan: 50000000, fact: 50500000, tone: 'amber', icon: 'person' },
  { key: 'tax', name: 'Налоги', color: '#F59E0B', plan: 30000000, fact: 27600000, tone: 'green', icon: 'star' },
  { key: 'rent', name: 'Аренда', color: '#E0384B', plan: 25000000, fact: 24750000, tone: 'green', icon: 'home' },
  { key: 'marketing', name: 'Маркетинг', color: '#A855F7', plan: 10000000, fact: 13200000, tone: 'red', icon: 'campaign', warn: true },
  { key: 'other', name: 'Прочее', color: '#94A3B8', plan: 10000000, fact: 8800000, tone: 'green', icon: 'more_horiz' },
])
...
const statusLabel = (tone) => (tone === 'red' ? 'Перерасход' : tone === 'amber' ? 'На грани' : 'В норме')
...
function buildChartData() {
  ...
  return {
    labels: days,
    datasets: [
      { label: 'План', data: plan, borderColor: '#94A3B8', borderDash: [6, 4], borderWidth: 2, tension: 0.35, pointRadius: 0, fill: false },
      { label: 'Факт', data: fact.concat(Array(31 - fact.length).fill(null)), borderColor: '#003D7C', backgroundColor: 'rgba(0,166,81,.13)', borderWidth: 2.5, tension: 0.35, pointRadius: 0, fill: true },
      { label: 'Прогноз', data: forecast, borderColor: '#003D7C', borderDash: [5, 4], borderWidth: 2, tension: 0.35, pointRadius: 0, fill: false },
    ],
  }
}
...
<template>
  <FcHeader
    title="Планирование"
    :pills="[{key:'month',label:'Месяц'},{key:'quarter',label:'Квартал'},{key:'year',label:'Год'}]"
    active-pill="month"
    :secondary-action="{ label:'Редактировать бюджет', icon:'edit' }"
    :primary-action="{ label: saveSuccess ? '✓ План сохранён' : 'Создать план', icon: saveSuccess ? 'check' : 'add' }"
    @primary="openModal"
  />

  <div class="fc-content">
    <div class="fc-page-title">
      <div class="eyebrow">FinControl / Планирование</div>
      <h1>Бюджет и план-факт</h1>
      <p class="subtitle"><span class="fc-green-line"></span>Апрель 2026 · Контроль планов по категориям</p>
    </div>
    ...
    <div v-if="showWarning" class="fc-banner amber" style="margin-bottom:18px;align-items:center">
      ...
      <div style="flex:1">
        <b>Внимание: риск кассового разрыва</b>
        <div style="font-size:12.5px;color:#6B7A99;margin-top:2px">Возможен кассовый разрыв до конца периода при текущей динамике расходов.</div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:18px">
      <div class="fc-kpi" style="border-top:3px solid #00A651">
        <div class="flex items-center justify-between" style="margin-bottom:8px">
          ...
          <span class="fc-badge green">91% выполнено</span>
        </div>
        <div class="fc-kpi-label">План доходов</div>
        <div class="flex items-baseline gap-2">
          <div class="fc-kpi-value green" style="font-size:22px">320 000 000</div>
          <span style="font-size:11px;color:#6B7A99">/ план 350 М UZS</span>
        </div>
        <div class="fc-bar" style="margin-top:10px"><div class="fc-bar-fill green" style="width:91%"></div></div>
        <div style="font-size:11px;color:#00A651;font-weight:700;margin-top:6px">✓ 91% выполнено</div>
      </div>
      ...
      <div class="fc-kpi" style="border-top:3px solid #E0384B;background:rgba(224,56,75,.03)">
        <div class="flex items-center justify-between" style="margin-bottom:8px">
          ...
          <span class="fc-badge red">Перерасход</span>
        </div>
        <div class="fc-kpi-label">План расходов</div>
        <div class="flex items-baseline gap-2">
          <div class="fc-kpi-value red" style="font-size:22px">198 000 000</div>
          <span style="font-size:11px;color:#6B7A99">/ план 180 М UZS</span>
        </div>
        ...
        <div style="font-size:11px;color:#E0384B;font-weight:700;margin-top:6px"><AppIcon name="warning" style="font-size:12px;vertical-align:middle" /> Перерасход 18 000 000 UZS (110%)</div>
      </div>
      ...
      <div class="fc-kpi" style="border-top:3px solid #003D7C">
        <div class="flex items-center justify-between" style="margin-bottom:8px">
          ...
          <span class="fc-badge blue">13 дней</span>
        </div>
        <div class="fc-kpi-label">До конца периода</div>
        <div class="fc-kpi-value navy" style="font-size:32px">13 дней</div>
        <div style="font-size:11px;color:#6B7A99;margin-top:2px">Остаток на счетах: <b>842 500 000 UZS</b></div>
        ...
        <div class="flex items-center justify-between" style="font-size:10.5px;color:#6B7A99;margin-top:4px">
          <span>День 18</span><span style="color:#003D7C;font-weight:700">Сегодня</span><span>День 31</span>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:16px;margin-bottom:18px">
      <div class="fc-card">
        <div class="fc-card-header">
          <div>
            <div class="fc-card-title">График план vs факт</div>
            <div class="fc-card-sub">Апрель 2026 — день 18 из 31</div>
          </div>
          <div style="display:flex;background:#F0F4FA;border-radius:8px;padding:2px">
            <button class="fc-period-pill" :class="{active:chartTab==='income'}" @click="chartTab='income'" style="font-size:11px;padding:4px 10px;border-radius:6px">Доходы</button>
            <button class="fc-period-pill" :class="{active:chartTab==='expense'}" @click="chartTab='expense'" style="font-size:11px;padding:4px 10px;border-radius:6px">Расходы</button>
            <button class="fc-period-pill" :class="{active:chartTab==='cf'}" @click="chartTab='cf'" style="font-size:11px;padding:4px 10px;border-radius:6px">Cash Flow</button>
          </div>
        </div>
        <div style="display:flex;gap:14px;margin-bottom:8px;font-size:12px;color:#6B7A99">
          <span><span style="display:inline-block;width:18px;height:2.5px;background:#003D7C;margin-right:6px;vertical-align:middle"></span>Факт</span>
          <span><span style="display:inline-block;width:18px;height:2.5px;border-top:2px dashed #94A3B8;margin-right:6px;vertical-align:middle"></span>План</span>
          <span><span style="display:inline-block;width:18px;height:2.5px;border-top:2px dashed #003D7C;margin-right:6px;vertical-align:middle"></span>Прогноз</span>
        </div>
      ...
      <div class="fc-card">
        <div class="fc-card-header">
          <div>
            <div class="fc-card-title">Бюджет по категориям</div>
        ...
        <div style="display:flex;flex-direction:column;gap:12px">
          <div v-for="c in categories" :key="c.key"
            ...
            <div class="flex items-center gap-2" style="margin-bottom:6px">
              ...
              <span style="flex:1;font-weight:600;font-size:13px;color:#1A2B4A">{{ c.name }}</span>
    ...
    <div class="fc-card" style="padding:0;overflow:hidden">
      <div class="fc-card-header" style="padding:18px 18px 14px">
        <div>
          <div class="fc-card-title">Детальный бюджет по статьям</div>
          <div class="fc-card-sub">Плановые суммы доступны для редактирования</div>
        </div>
        <div class="flex items-center gap-2">
          <span style="font-size:11px;color:#6B7A99">Все суммы в UZS</span>
          <button class="fc-cta-ghost" style="height:30px;padding:0 10px;font-size:11px"><AppIcon name="file_download" /> Экспорт</button>
        </div>
      </div>
      <table class="fc-table">
        <thead>
          <tr><th>Категория</th><th style="text-align:right">Плановая сумма</th><th style="text-align:right">Факт</th><th style="text-align:right">Остаток</th><th style="text-align:right">%</th><th>Статус</th></tr>
        </thead>
        <tbody>
          ...
          <tr style="background:#F8FAFC;font-weight:700">
            ...
            <td><span class="fc-badge red">Перерасход</span></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showModal" style="position:fixed;inset:0;background:rgba(15,23,42,.5);backdrop-filter:blur(4px);z-index:200;display:flex;align-items:center;justify-content:center" @click.self="showModal = false">
      <div style="background:white;border-radius:14px;padding:22px;width:540px;max-width:92%;max-height:90vh;overflow:auto">
        <div class="flex items-center justify-between" style="margin-bottom:16px">
          <h3 class="fc-card-title" style="font-size:17px">Создать план</h3>
          <button class="fc-icon-btn" @click="showModal = false"><AppIcon name="close" /></button>
        </div>

        <div style="font-size:11px;color:#6B7A99;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">Тип периода</div>
        <div style="display:flex;background:#F0F4FA;border-radius:9px;padding:3px;margin-bottom:14px">
          <button v-for="p in [{k:'month',l:'Месяц'},{k:'quarter',l:'Квартал'},{k:'year',l:'Год'}]" :key="p.k"
        ...
        <div class="grid grid-cols-2 gap-3" style="margin-bottom:14px">
          <div class="fc-form-field"><label>Начало периода</label><input class="fc-input" type="date" value="2026-05-01" /></div>
          <div class="fc-form-field"><label>Конец периода</label><input class="fc-input" type="date" value="2026-05-31" /></div>
        </div>

        <div style="font-size:13px;font-weight:700;color:#1A2B4A;margin-bottom:8px">Плановый бюджет по категориям</div>
        ...
        <div style="background:rgba(0,61,124,.08);padding:12px 14px;border-radius:10px;display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <span style="font-size:12px;color:#003D7C;font-weight:600">Итоговый план расходов</span>
          <span class="fc-num" style="color:#003D7C;font-size:18px">{{ fmt(modalTotal) }} UZS</span>
        </div>

        <div class="flex gap-3 justify-end">
          <button class="fc-cta-ghost" @click="showModal = false">Отмена</button>
          <button class="fc-cta-primary" @click="saveModal">Сохранить план</button>
        </div>
      </div>
    </div>
  </div>
